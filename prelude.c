// -include prelude.c --- stuff to appear at the *top* of each TU written in "Common-Platform" C (CP/C)
...
# include <float.h>
// hopefully the C99 macros like `bool` or `static_assert` are not brought in!
...
_Static_assert( // deliberately relaxed for C (as opposed to C++) to accommodate some legacy targets
...
# if !__linux__ && !__FreeBSD__
   ...
   # if !(__GLIBC__ > 2 || __GLIBC__ == 2 && __GLIBC_MINOR__ >= 25)
      _Static_assert(0, "Unsupported target libc (AKA C runtime) detected");
   # endif
# endif
