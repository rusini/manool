...
namespace mnl::aux {
   ...
   template<template<class> class Expr> static MNL_INLINE inline code optimize(Expr<> &expr) {
      ...
      {  const auto optimize = [&](auto op) MNL_INLINE{ return
            ...
            match< expr_apply<2, expr_lit<decltype(op)>, expr_lit<std::string>,        expr_tvar> >                    (expr, res) ||
            match< expr_apply<2, expr_lit<decltype(op)>, expr_lit<std::string>,        code> >                         (expr, res) ||
            match< expr_apply<2, expr_lit<decltype(op)>, expr_lit<>,                   expr_tvar> >                    (expr, res) ||
            match< expr_apply<2, expr_lit<decltype(op)>, expr_lit<>,                   code> >                         (expr, res) ||
         ...
         if ( optimize(op<sym::id("==")>) ||
              optimize(op<sym::id("<>")>) ) return res;
      }
      {  const auto optimize = [&](auto op) MNL_INLINE{
            return
               match< expr_apply<2, expr_lit<decltype(op)>, expr_lit<long long>,          expr_tvar> >                    (expr, res) ||
